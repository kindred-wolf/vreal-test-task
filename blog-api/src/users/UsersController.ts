import { Controller, Delete, Get, Param, UseGuards, Request, Put, Body, ForbiddenException } from "@nestjs/common"
...
export class UsersController{
    ...
    @Roles(UserRole.USER, UserRole.ADMIN)
    @Put(':id')
    async updateUser(@Param('id') id: string, @Request() req, @Body() updatedUser: UpdateUserDataDto){
        console.clear()
        console.log('id: ' + id + '\nreq: ' + req.user.id)
        if(id != req.user.id && req.user.role !== UserRole.ADMIN){
            throw new ForbiddenException()
